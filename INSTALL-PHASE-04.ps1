...
Write-Host "AngelBird Phase 04 - clean Expo SDK 57 setup" -ForegroundColor Cyan
...
Write-Host "Removing stale Phase 01-03 dependency state..." -ForegroundColor Yellow
...
if (Test-Path ".env") {
  Copy-Item ".env" ".env.phase03.backup" -Force
}
Set-Content ".env" "EXPO_PUBLIC_API_BASE_URL=https://angelbirdanalysis-api.vercel.app`r`n"
Write-Host "Mobile API locked to deployed AngelBird Vercel API (previous .env backed up when present)." -ForegroundColor Green
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "Expo reported a dependency mismatch. Applying Expo-compatible fixes..." -ForegroundColor Yellow
...
Write-Host "" 
Write-Host "Phase 04 verification complete." -ForegroundColor Green
