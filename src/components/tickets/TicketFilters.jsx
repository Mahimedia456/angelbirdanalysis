...
import { ALLOWED_REGIONS, getUniqueValues } from "../../utils/ticketMapper";

export default function TicketFilters({ tickets = [], filters, onChange }) {
  ...
  return (
    <div className="angel-card p-5">
      <div className="grid gap-4 xl:grid-cols-[1.4fr_0.7fr_0.9fr_0.9fr]">
        <div>
          <label className="angel-label">Search Ticket / Product / Subject</label>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[0.9fr_0.6fr_0.6fr_auto] xl:items-end">
        <div>
          ...
          <select
            ...
            onChange={(event) => updateFilter("procedure", event.target.value)}
          >
        ...
        <button onClick={resetFilters} className="angel-btn angel-btn-dark">
          Reset Filters
        </button>
      </div>
    </div>
